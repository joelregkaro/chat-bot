  ...
  return (
    <section className="py-16 bg-gradient-to-br from-light-orange via-blue-50/30 to-orange-50/30 relative overflow-hidden">
      ...
      <div className="container mx-auto relative z-10">
        <h2 className="text-4xl font-bold text-blue text-center mb-3">What Our Users Say</h2>
        ...
        <div className="grid md:grid-cols-3 gap-8 px-4 cursor-pointer">
          {testimonials.slice(currentSlide, currentSlide + 3).map((testimonial, index) => (
            <div
              key={index}
              className={`border rounded-xl p-6 shadow-lg bg-white hover:shadow-xl transition-all duration-500 transform hover:-translate-y-2 ${
                currentSlide + index === currentSlide ? 'ring-2 ring-blue scale-105' : ''
              }`}
              style={{
                transition: 'all 0.5s ease-in-out',
                opacity: currentSlide + index === currentSlide ? 1 : 0.8,
                transform: `translateY(${currentSlide + index === currentSlide ? '-8px' : '0'})`
              }}
            >
              <div className="flex items-center mb-5">
                <div className="mr-3">
                  <img
                    ...
                    width={60}
                    height={60}
                    className="rounded-full border-4 border-blue/20"
                  />
                </div>
                <div>
                  <h4 className="font-semibold text-darkgray text-lg">{testimonial.name}</h4>
                  <p className="text-darkgray/70 text-sm">{testimonial.role}</p>
                </div>
              </div>
              ...
              <p className="text-darkgray text-base mb-5 leading-relaxed italic">
                "{testimonial.text}"
              </p>
              <div className="flex items-center text-sm text-darkgray/70 pt-3 border-t">
                <span className="font-medium">Tool used:</span>
                <span className="ml-2 text-blue font-semibold">{testimonial.tool}</span>
              </div>
            </div>
          ))}
        </div>
            ...
            {testimonials.map((_, index) => (
              <button
                ...
                className={`h-3 rounded-full transition-all duration-300 ${
                  currentSlide === index ? "bg-blue w-6" : "bg-gray-300 hover:bg-gray-400 w-3"
                }`}
