...
export default function StickyChat({ onClose }: StickyChatProps) {
  ...
  const initializeRazorpay = async (paymentLink: string): Promise<boolean> => {
    try {
      ...
      try {
        ...
        // Method 2: Extract from RegisterKaro-specific path format
          ...
          if (pathParts.length > 1) {
            orderId = pathParts[1];
            console.log(
              `✅ Extracted order ID from RegisterKaro path: ${orderId}`
            );
          }
        }
        ...
        // Handle case where direct Razorpay order URL is provided
          ...
          if (orderMatches && orderMatches[1]) {
            ...
            console.log(
              `✅ Extracted order ID from direct Razorpay URL: ${orderId}`
            );
          }
        }
        ...
        if (!orderId) {
          console.error(
            "❌ Failed to extract order_id from payment link:",
            paymentLink
          );
      ...
      if (hasCompletedPayment) {
        console.log(
          "✅ Payment already completed, not showing Razorpay checkout again"
        );
        sendChatMessage(
          "Our records show you've already completed payment. If you need assistance with anything else, please let me know."
        );
        return true;
      }
      ...
      console.log(
        `⚙️ Is constructor? ${
          typeof window.Razorpay === "function" ? "Yes" : "No"
        }`
      );
      ...
      return new Promise((resolve) => {
        try {
          ...
          const key =
            process.env.REACT_APP_RAZORPAY_KEY_ID || "rzp_test_I98HfDwdi2qQ3T"; // Fallback for safety
          console.log(
            `🔑 Using Razorpay key: ${
              process.env.REACT_APP_RAZORPAY_KEY_ID
                ? process.env.REACT_APP_RAZORPAY_KEY_ID + " [From ENV]"
                : "rzp_test_I98HfDwdi2qQ3T [Fallback]"
            }`
          );

          console.log(
            `🚀 Initializing Razorpay checkout with order ID: ${orderId}`
          );
          ...
          const options = {
            ...
            modal: {
              ondismiss: function () {
                console.log(
                  "⚠️ Payment popup closed without completing payment"
                );
                sendChatMessage(
                  "I see you've closed the payment window. If you need any more information before proceeding with the payment, please let me know."
                );
                resolve(false);
              },
            ...
            handler: function (response: any) {
              console.log(
                "💰 Payment potentially successful - will verify with backend",
                response
              );
              ...
              if (response.razorpay_payment_id && response.razorpay_order_id) {
                ...
                try {
                  ...
                  console.error(
                    "Could not save payment info to localStorage",
                    e
                  );
                }
                ...
                sendChatMessage(
                  "Great! I'm verifying your payment. This should just take a moment..."
                );
                resolve(true);
              } else {
                console.error(
                  "❌ Payment handler called with invalid response:",
                  response
                );
                sendChatMessage(
                  "I received a payment notification, but some details are missing. Please wait while I verify your payment status."
                );
                resolve(false);
              }
            },
          ...
          try {
            ...
            if (typeof window.Razorpay !== "function") {
              console.error(
                "❌ window.Razorpay is not a constructor function:",
            ...
            if (
              !rzp ||
              typeof rzp.on !== "function" ||
              typeof rzp.open !== "function"
            ) {
            ...
            rzp.on("payment.error", function (error: any) {
              ...
              sendChatMessage(
                "We encountered an error processing your payment. Let me help you try again or explore alternative payment options."
              );
              resolve(false);
            });
            ...
            rzp.on("payment.failed", function (response: any) {
              ...
              sendChatMessage(
                "It looks like there was an issue with the payment. Would you like to try again or use a different payment method?"
              );
              resolve(false);
            });
            ...
            console.error(
              "❌ Detailed error:",
              rzpInstanceError instanceof Error
                ? rzpInstanceError.message
                : "Unknown error"
            );
            resolve(false);
          }
  ...
  const openPaymentIframe = (paymentLink: string): Promise<boolean> => {
    return new Promise((resolve) => {
      console.log(
        "⚠️ Using fallback iframe payment method for link:",
        paymentLink
      );
      ...
      try {
        ...
        sendChatMessage(
          "I've opened Razorpay's secure payment window for you. You can complete your payment there and then return to this chat when done."
        );
          ...
          try {
            ...
            if (event.data) {
              ...
              if (typeof event.data === "string") {
                if (
                  event.data.includes("razorpay_payment_id") ||
                  event.data.includes("payment_success") ||
                  event.data.includes("payment_complete")
                ) {
                  console.log(
                    "✅ Payment completion detected via iframe message (string)"
                  );
              ...
              // Handle object message type
                ...
                if (
                  ...
                  console.log(
                    "✅ Payment completion detected via iframe message (object):",
                    event.data
                  );
                  handleSuccessfulPayment();
                }
        ...
        sendChatMessage(
          "I'm having trouble opening the payment page. Please try again later or contact our support team for assistance."
        );
        resolve(false);
      }
    });
